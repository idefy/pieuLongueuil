...
async function loadData() {
  try {
    ...
    pagesData           = Array.isArray(pi) ? pi : (pi?.pages_index || []);
...
function renderAll() {
  renderHero(); renderQuickStrip(); renderRessources(); renderPastoral(); renderDirigeants(); renderOrganisations(); renderFooter(); renderSiteStrings(); renderNavDropdown(); updateI18n();
  initScrollEffects();
}
...
function renderHero() {
  ...
  const heroName = cfg.stakeNameI18n?.[l] || cfg.stakeNameI18n?.fr || cfg.stakeName;
  setEl('hero-title', heroName);
...
function renderSiteStrings() {
  ...
  // Section eyebrows — safe fallbacks if not yet loaded
  const eb = cfg.eyebrows || {};
  setEl('eyebrow-membres',    (eb.membres    && (eb.membres[l]    || eb.membres.fr))    || 'Membres');
  setEl('eyebrow-pastoral',   (eb.pastoral   && (eb.pastoral[l]   || eb.pastoral.fr))   || 'Service pastoral');
  setEl('eyebrow-dirigeants', (eb.dirigeants && (eb.dirigeants[l] || eb.dirigeants.fr)) || 'Dirigeants');
  setEl('eyebrow-pieu',       (eb.pieu       && (eb.pieu[l]       || eb.pieu.fr))       || 'Pieu Longueuil');
  ...
  try {
    ...
    if (pq && pq.text) {
      ...
      const htmlText = rawText
        .replace(boldRe, '<strong style="color:var(--c-secondary)">$1</strong>')
        .split('\n\n').join('<br><br>');
      const el = document.getElementById('pastoral-quote-text');
      if (el) el.innerHTML = htmlText;
      const attr = pq.attribution ? (pq.attribution[l] || pq.attribution.fr || '') : '';
      setEl('pastoral-quote-attr', attr);
    }
  } catch(e) { console.warn('pastoralQuote render error', e); }

  // Stake name in footer
  const name = (cfg.stakeNameI18n && (cfg.stakeNameI18n[l] || cfg.stakeNameI18n.fr)) || cfg.stakeName || 'Pieu Longueuil';
